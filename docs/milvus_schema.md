...
| `identity_attributes_json` / `variant_attributes_json` / `descriptive_attributes_json` | JSON | 身份/变体/描述属性（§14.1） |
| `price` / `original_price` / `shipping_fee` / `coupon_amount` / `currency` | FLOAT/VARCHAR | 价格（实付 = price − coupon + shipping，§15.1） |
...
- 输入与本地降级同源的**只读商品快照**（JSONL，每行一个 Offer，§13.7）。
...
- **metadata**：`filter` 表达式实现硬过滤（与 `offer_matches_hard_filters`
  同一语义，§13.4）——平台、价格区间、品牌、型号、评分/销量下限。
