...
`JD_PARAM_JSON_NAME` 默认为京东 JOS 公共参数字段 `360buy_param_json`，通常不需要修改；只有对接兼容网关时才建议覆盖。京东响应会兼容 `_response` 与部分平台历史上出现过的 `_responce` 包装字段；价格展示会优先使用 `lowestCouponPrice` / `lowestPrice` 这类到手价字段。
