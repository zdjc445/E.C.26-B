...
def _schema(dim: int, image_dim: int | None) -> tuple[CollectionSchema, _IndexParamsLike]:
    ...
    fields = [
        ...
        varchar("source_sku_id", 128),
        varchar("source_offer_id", 128),
        varchar("record_kind", 32),
        varchar("raw_category_path", 512),
        FieldSchema(name="raw_attributes_json", dtype=DataType.JSON),
        varchar("provenance", 32),
        varchar("source_revision", 128),
        varchar("source_content_hash", 64),
        varchar("availability", 32),
        varchar("price_basis", 32),
        varchar("source_updated_at", 32),
        varchar("data_version", 64),
        varchar("title", 1024),
        varchar("normalized_title", 1024),
        varchar("search_text", 16384),
        varchar("search_text_hash", 64),
