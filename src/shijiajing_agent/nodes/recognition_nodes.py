"""识别节点：VLM 识别、用户修正、识别标准化（方案 §9.2、§11.2、§12.3）。
...
from shijiajing_agent.nodes.node_support import set_dirty, timed
...
def make_recognize_image_node(deps: Any) -> Any:
    ...
    async def recognize_image_node(state: AgentState) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "errors": [
                    *errors,
                    {"node_name": "recognize_image", "error_code": str(code), "message": str(exc)},
                ],
...
def make_apply_correction_node(deps: Any) -> Any:
    """用户修正（§6.3）：recognition_id 不一致直接拒绝。"""
    ...
    async def apply_correction_node(state: AgentState) -> dict[str, Any]:
        ...
        return {"next_action": "correction_applied", **set_dirty(state, *_DOWNSTREAM_DIRTY)}

    return apply_correction_node


def make_normalize_recognition_node(deps: Any) -> Any:
    """识别结果标准化（§12.3）：未知值置空并记录 notice。"""
    ...
    async def normalize_recognition_node(state: AgentState) -> dict[str, Any]:
        ...
        return {"notices": notices, "next_action": "normalized"}
