...
比价排序 → 多轮筛选修正 的**可恢复 Workflow Agent**（LangGraph）。

工程只实现 Agent 逻辑，不包含 Web API 与客户端（方案 §25）。
...
uv run shijiajing-eval --live --freeze-dir <dir>   # 真实数据实时评测并冻结
```

- 11 项指标 + §22.3 阈值门禁；4 项**阻断指标**未达标（含未测量）即失败。
- 退出码：0 达标 / 1 阻断失败 / 2 配置或数据集错误。
- 详见 [docs/evaluation.md](docs/evaluation.md)。

## 数据状态（请先阅读）

**仓库内没有真实电商数据。** 按方案 §25 明确披露如下：

| 数据 | 状态 |
|---|---|
| 商品快照 / 商品 | **样例数据**（测试夹具与评测种子数据集），不是任何真实平台数据；平台使用契约 ID（taobao/jd/pinduoduo） |
| `src/shijiajing_agent/data/eval/` 评测集 | **CI 回归种子样例**，不是 §22.3 正式冻结评测集；`reports/frozen_eval_report.md` 是基于种子数据的回归报告，不代表真实商品上的达标 |
| 模型输出（`recorded` 字段） | 冻结的样例输出；需要模型调用次数插桩的指标如实标注 `pending` |
| 真实数据接入 | `shijiajing-index-products` 索引真实快照 → Milvus；`shijiajing-eval --live --freeze-dir` 产出正式评测集（需人工复核标注） |
...
```
输入 → 识别/修正 → 意图+约束 → 查询改写 → 混合召回 → 同款匹配 → SKU 拆分
     → 排序 → 证据 → 解释（事实校验） → 响应；多轮经 Checkpoint 恢复
```
...
uv run pytest -q              # 248 个离线测试；集成测试（-m integration）需要真实外部资源
```
