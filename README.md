...
**当前阶段：聊天式 AI 识别 + 公开样例商品 + 多平台 Mock 推荐 + 7 维度自然语言筛选 + 动态建议卡。**
...
- 公开 Flipkart 样例商品：带商品图、商品链接、标价和折扣价
- 多平台 Mock 商品回退：`京东-mock`、`拼多多-mock`、`淘宝-mock`
...
- 商品搜索和比价默认合并离线公开样例与人工构造 Mock 数据，不调用真实电商接口。旧 Mock 数据不代表真实平台数据。
...
- Flutter test：27 widget tests，全部通过
